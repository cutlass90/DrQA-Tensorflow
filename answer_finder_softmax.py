...
class AnswerFinder(BaseModel):
    ...
    def _create_metrics(self):
        ...
        precision, recall, f1 = metrics.pr_re_f1_multiclass(
            labels=self.target_start, logits=self.logits_start)
        self.start_precision = tf.reduce_mean(precision)
        self.start_recall = tf.reduce_mean(recall)
        self.start_f1 = tf.reduce_mean(f1)

        precision, recall, f1 = metrics.pr_re_f1_multiclass(
            labels=self.target_end, logits=self.logits_end)
        self.end_precision = tf.reduce_mean(precision)
        self.end_recall = tf.reduce_mean(recall)
        self.end_f1 = tf.reduce_mean(f1)

    def _create_summaries(self):
        print('_create_summaries')
        with tf.name_scope('start'):
            tf.summary.scalar('loss', self.start_cost)
            tf.summary.scalar('accuracy', self.start_acc)
            tf.summary.scalar('precission', self.start_precision)
            tf.summary.scalar('recall', self.start_recall)
            tf.summary.scalar('f_score', self.start_f1)
        
        with tf.name_scope('end'):
            tf.summary.scalar('loss', self.end_cost)
            tf.summary.scalar('accuracy', self.end_acc)
            tf.summary.scalar('precission', self.end_precision)
            tf.summary.scalar('recall', self.end_recall)
            tf.summary.scalar('f_score', self.end_f1)
    ...
    def _train_step(self, loader):
        ...
        feedDict = {
            ...
            self.target_start : data[6],
            self.target_end : data[7],
